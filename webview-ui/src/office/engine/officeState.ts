...
export class OfficeState {
  ...
  /** Stable order projects were first seen — drives each project's seat anchor. */
  projectOrder: string[] = []
  ...
  /** Pick a free seat closest to a project's anchor column so same-project
   *  agents cluster together into a zone. Falls back to any free seat. */
  private findSeatForProject(projectName: string): string | null {
    if (!this.projectOrder.includes(projectName)) this.projectOrder.push(projectName)
    const idx = this.projectOrder.indexOf(projectName)
    const n = Math.max(this.projectOrder.length, 1)
    const anchorCol = Math.floor(((idx + 0.5) / n) * this.layout.cols)
    let best: string | null = null
    let bestDist = Infinity
    for (const [uid, seat] of this.seats) {
      if (seat.assigned) continue
      // Column proximity dominates; row is a gentle tiebreak (fill top-down).
      const dist = Math.abs(seat.seatCol - anchorCol) * 1000 + seat.seatRow
      if (dist < bestDist) {
        bestDist = dist
        best = uid
      }
  ...
  addAgent(id: number, preferredPalette?: number, preferredHueShift?: number, preferredSeatId?: string, skipSpawnEffect?: boolean, folderName?: string, projectName?: string, projectColor?: string): void {
    ...
    // Project agents cluster by zone (ignore persisted seat so the zone layout
    // takes effect); others honor their preferred seat, then any free seat.
    let seatId: string | null = null
    if (projectName) {
      seatId = this.findSeatForProject(projectName)
    } else {
